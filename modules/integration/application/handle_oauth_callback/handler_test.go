...
func newSigner(t *testing.T, ttl time.Duration) services.StateSignerPort {
	t.Helper()
	return newLocalStateSigner(bytes.Repeat([]byte{0x42}, 32), ttl)
}
...
func TestHandleCallback_NewIntegration_HappyPath(t *testing.T) {
	...
	if resp.ReturnHost != "http://acme.pulzifi.lvh.me:3000" {
		t.Errorf("ReturnHost = %q, want http://acme.pulzifi.lvh.me:3000", resp.ReturnHost)
	}
