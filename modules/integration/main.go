...
func main() {
	cfg := config.Load()
	logger.Info("Starting Integration Service", zap.String("config", cfg.String()))

	router := gin.Default()
	router.GET("/health", middleware.HealthCheck())

	router.GET("/api/integrations", func(c *gin.Context) {
		c.JSON(200, gin.H{"message": "list integrations"})
	})

	router.POST("/api/integrations", func(c *gin.Context) {
		c.JSON(201, gin.H{"message": "create integration"})
	})
