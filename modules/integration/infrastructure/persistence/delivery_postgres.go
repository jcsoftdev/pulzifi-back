...
func (r *DeliveryPostgresRepository) GetByID(ctx context.Context, id uuid.UUID) (*entities.Delivery, error) {
	...
	err := database.WithTenant(ctx, r.db, r.tenant, func(tx *sql.Tx) error {
		...
		if err != nil {
			...
			return fmt.Errorf("delivery repo: get by id: %w", err)
		}
		result = d
		return nil
	})
...
func (r *DeliveryPostgresRepository) ClaimPending(ctx context.Context, limit int, now time.Time) ([]*entities.Delivery, error) {
	...
	var result []*entities.Delivery
	err := database.WithTenant(ctx, r.db, r.tenant, func(tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx, q, limit, now)
		if err != nil {
			return fmt.Errorf("delivery repo: claim pending: %w", err)
		}
		...
		for rows.Next() {
			...
			if err != nil {
				return fmt.Errorf("delivery repo: claim pending scan: %w", err)
			}
			result = append(result, d)
		}
		if err := rows.Err(); err != nil {
			return fmt.Errorf("delivery repo: claim pending rows: %w", err)
		}
		return nil
	})
	if err != nil {
...
func (r *DeliveryPostgresRepository) MarkDelivered(ctx context.Context, id uuid.UUID, code int, bodySnip string) error {
	...
	return database.WithTenant(ctx, r.db, r.tenant, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, q, id, code, bodySnip)
		if err != nil {
...
func (r *DeliveryPostgresRepository) MarkFailed(ctx context.Context, id uuid.UUID, code *int, body, errMsg string, nextAttempt time.Time, history []entities.Attempt) error {
	...
	return database.WithTenant(ctx, r.db, r.tenant, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, q, id, nullCode, body, errMsg, nextAttempt, histJSON)
		if err != nil {
...
func (r *DeliveryPostgresRepository) MarkDead(ctx context.Context, id uuid.UUID, errMsg string, history []entities.Attempt) error {
	...
	return database.WithTenant(ctx, r.db, r.tenant, func(tx *sql.Tx) error {
		_, err := tx.ExecContext(ctx, q, id, errMsg, histJSON)
		if err != nil {
...
func (r *DeliveryPostgresRepository) ListByDestination(ctx context.Context, destinationID uuid.UUID, limit, offset int) ([]*entities.Delivery, error) {
	...
	var result []*entities.Delivery
	err := database.WithTenant(ctx, r.db, r.tenant, func(tx *sql.Tx) error {
		rows, err := tx.QueryContext(ctx, q, destinationID, limit, offset)
		if err != nil {
			return fmt.Errorf("delivery repo: list by destination: %w", err)
		}
		defer func() { _ = rows.Close() }()
		...
		for rows.Next() {
			d, err := scanDelivery(rows)
			if err != nil {
				return fmt.Errorf("delivery repo: list by destination scan: %w", err)
			}
			result = append(result, d)
		}
		if err := rows.Err(); err != nil {
			return fmt.Errorf("delivery repo: list by destination rows: %w", err)
		}
		return nil
	})
	if err != nil {
