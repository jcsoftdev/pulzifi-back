...
func main() {
	cfg := config.Load()
	logger.Info("Starting Page Service", zap.String("config", cfg.String()))

	router := gin.Default()
	router.GET("/health", middleware.HealthCheck())

	router.GET("/api/pages", func(c *gin.Context) {
		c.JSON(200, gin.H{"message": "list pages"})
	})

	router.POST("/api/pages", func(c *gin.Context) {
		c.JSON(201, gin.H{"message": "create page"})
	})
