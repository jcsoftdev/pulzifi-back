...
func main() {
	cfg := config.Load()
	logger.Info("Starting Alert Service", zap.String("config", cfg.String()))

	router := gin.Default()
	router.GET("/health", middleware.HealthCheck())

	router.GET("/api/alerts", func(c *gin.Context) {
		c.JSON(200, gin.H{"message": "list alerts"})
	})

	router.POST("/api/alerts", func(c *gin.Context) {
		c.JSON(201, gin.H{"message": "create alert"})
	})
