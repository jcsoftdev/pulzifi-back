...
func main() {
	cfg := config.Load()
	logger.Info("Starting Usage Service", zap.String("config", cfg.String()))

	router := gin.Default()
	router.GET("/health", middleware.HealthCheck())

	router.GET("/api/usage", func(c *gin.Context) {
		c.JSON(200, gin.H{"message": "get usage"})
	})

	router.POST("/api/usage/track", func(c *gin.Context) {
		c.JSON(201, gin.H{"message": "track usage"})
	})
