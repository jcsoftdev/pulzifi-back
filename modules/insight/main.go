...
func main() {
	cfg := config.Load()
	logger.Info("Starting Insight Service", zap.String("config", cfg.String()))

	router := gin.Default()
	router.GET("/health", middleware.HealthCheck())

	router.GET("/api/insights", func(c *gin.Context) {
		c.JSON(200, gin.H{"message": "list insights"})
	})

	router.POST("/api/insights", func(c *gin.Context) {
		c.JSON(201, gin.H{"message": "create insight"})
	})
