...
function MetricCard({ card, index }: { card: ProblemCard; index: number }) {
    ...
    () => {
      ...
      gsap.to(obj, {
        ...
        onUpdate() {
          ...
          const display =
            parsed.value % 1 === 0 ? Math.round(obj.val).toString() : obj.val.toFixed(1)
  ...
  return (
    ...
    >
      <p ref={metricRef} className="font-heading text-5xl font-bold text-[var(--pz-accent)]">
      ...
      <p className="mt-3 text-base leading-relaxed text-[var(--pz-ink-2)]">{card.description}</p>
    </AnimatedSection>
  )
}
