...
export async function proxy(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Proxy-${requestId}] Session found: user=${session.user?.email}, tenant=${session.tenant}`)
    ...
    if (!isPublicPath) {
      ...
      console.log(`[Proxy-${requestId}] Hostname: ${hostname}, Subdomain tenant: ${subdomainTenant}`)
      ...
      if (subdomainTenant && subdomainTenant !== 'app' && session.tenant && subdomainTenant !== session.tenant) {
        console.warn(`[Proxy-${requestId}] Tenant mismatch: session=${session.tenant}, subdomain=${subdomainTenant}`)
        const loginUrl = new URL('/login', request.url)
    ...
    const totalDuration = performance.now() - startTime
    console.error(`[Proxy-${requestId}] ERROR ${method} ${path} in ${totalDuration.toFixed(2)}ms:`, error)
