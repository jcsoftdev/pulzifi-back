...
import type { AnimationKeyframes, IAnimationStrategy } from '../../domain/ports/animation'
...
export class MorphAnimationStrategy implements IAnimationStrategy {
  ...
  enter(element: HTMLElement, triggerRect?: TriggerRect, targetRect?: DOMRect): AnimationKeyframes {
    ...
    const translateX =
      triggerRect.left - targetRect.left + (triggerRect.width - targetRect.width) / 2
    const translateY =
      triggerRect.top - targetRect.top + (triggerRect.height - targetRect.height) / 2
