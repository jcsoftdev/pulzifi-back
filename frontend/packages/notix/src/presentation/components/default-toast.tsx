...
export const DefaultToast = memo(function DefaultToast({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentional — only re-run when next changes, not on every open toggle
  useEffect(() => {
    ...
    >
        ...
        <div data-notix-header-stack>
          ...
          >
            <div
              ...
              aria-hidden="true"
            >
              {headerLayer.current.view.icon ?? STATE_ICONS[headerLayer.current.view.state]}
            </div>
            <span
              data-notix-title
              data-state={headerLayer.current.view.state}
          ...
          {headerLayer.prev && (
            ...
            >
              <div
                ...
                aria-hidden="true"
              >
                {headerLayer.prev.view.icon ?? STATE_ICONS[headerLayer.prev.view.state]}
              </div>
              <span
                data-notix-title
                data-state={headerLayer.prev.view.state}
      ...
      {hasDesc && (
        <div data-notix-content data-visible={open}>
          <div ref={contentRef} data-notix-description className={view.styles?.description}>
            {view.description}
            {view.button && (
              <button
                type="button"
      ...
      {showProgress && hasDesc && duration != null && duration > 0 && !isLoading && (
        <div
          ...
          style={
            {
              '--notix-progress-duration': `${duration}ms`,
            } as CSSProperties
          }
        />
      )}
