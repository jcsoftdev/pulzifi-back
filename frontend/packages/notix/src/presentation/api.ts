...
export const notix = {
  ...
  promise: <T>(p: Promise<T> | (() => Promise<T>), opts: NotixPromiseOptions<T>): Promise<T> =>
    manager.promise(p, opts),
  dismiss: (id: ToastId): void => manager.dismiss(id),
  update: (id: ToastId, opts: Partial<NotixOptions>): void => manager.update(id, opts),
  clear: (position?: NotixPosition): void => manager.clear(position),
  setDefaultPosition: (position: NotixPosition): void => manager.setDefaultPosition(position),
  setDefaultOptions: (options: Partial<NotixOptions>): void => manager.setDefaultOptions(options),
}
